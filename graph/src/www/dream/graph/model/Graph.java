...
public class Graph {
	...
	public List<Vertex> dfs(Vertex vertex) {
		...
		while(!쌓여있는다음방문대상도시들.isEmpty()) { // 재귀함수
			...
			for (Edge 길 : 모든길) {
				Vertex 다른도시 = 길.getOhterVertex(다음도시);
	...
	public List<Vertex> bfs(Vertex vertex) {
		Queue<Vertex> 줄지어서기다리는다음방문대상도시들 = new LinkedList<>();
		줄지어서기다리는다음방문대상도시들.offer(vertex);
		List<Vertex> 차례로방문한도시목록 = new ArrayList<>();
		
		while (!줄지어서기다리는다음방문대상도시들.isEmpty()) {
			Vertex 다음도시 = 줄지어서기다리는다음방문대상도시들.poll();
			if (차례로방문한도시목록.contains(다음도시))
				continue;
			차례로방문한도시목록.add(다음도시);
