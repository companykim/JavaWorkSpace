...
public class RSC {
	...
	public static <T extends Comparable<T>> T[] rxSort(T[] data, int radix) {
		...
		int[] bucketEleCount = new int[bucketSize]; 
		 ...
		 // 해당 자릿수로 정렬한 결과
		T[] sortResult = Arrays.copyOf(data, dataSize);
		 
		int i, j, n;
		int curDigit;   // 현재 관심 자리
		int[] bucketId = new int[dataSize]; // 각 데이터 별 방 번호
		 // 메모리 할당
		for (n = 0; n < maxDigist; n++) { // 1의 자리, 10의자리, 100의 자리 순으로 진행
			for (i = 0; i < bucketSize; i++)
		       ...
		       // 각 숫자의 전체 관심 자리를 기준으로 병렬 발생횟수를 센다.
		       for (j = 0; j < dataSize; j++) {
		    	...
		    	bucketId[j] += radix - 1; // 음수 다루기 위한 방 번호 알기
